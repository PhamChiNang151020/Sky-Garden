...
struct Stage{int gd;};
Stage stage[12]; // 12 bông hoa có 4 gd (0 --> 4) mỗi khi hàm timer chạy xong 1 khoảng time nhất định
// Khai báo struct có 5 giá trị kiểu int // giá trị pla (1: đã đặt, 0: chưa đặt) biến của struct dùng cho cả hoa và chậu
struct Pot{int x,y,z,c,pla;};
//biến xác nhận hiện 2 btn xác nhận đặt chậu
int XV_plant = 0;
//thứ tự click đặt chậu
int arrowclick = -1;
// thứ tự click đặt của hoa trên các chậu - loai hoa
int flclick = 0;
...
void Exit();
void display();
void mouseClick_MainHandle(int button , int state, int x, int y);
...
void mousePlant_Pot(int button , int state, int x, int y);
void mousePlant_Flower(int button , int state, int x, int y);
void mousePlant_ChangeArr_Pot_Flower(int button , int state, int x, int y);
void Mission();
...
void mouseClick_MainHandle(int button , int state, int x, int y)
{
	...
	//Rct_btn_X = {630 +180 +180 + 90 ,Xsizex + 630 +180 +180 +90,Xsizey + 617, 617},
		...
		if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && x >= 630 +180 +180 + 90 
			&& x <= Xsizex + 630 +180 +180 +90 && y >= 617 && y <=  Xsizey + 617  && XV_plant == 1 && planting == false)
		{
			XV_plant = 0;
			yes_no = false;
			glutDisplayFunc(screenGame2);
	...
	// _Y Rct_btn_V = {810 +180 +180 ,Vsizex + 810 +180 +180,Vsizey + 616, 616}
		if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && x >= 810 +180 +180 
			&& x <= Vsizex + 810 +180 +180 && y >= 616 && y <=  Vsizey + 616  && XV_plant == 1 && planting == false)
		{
			XV_plant = 0;
			yes_no = true;
			glutDisplayFunc(screenGame2);
...
void screenGame()
{
	...
	glutMouseFunc(mouseClick_MainHandle);
